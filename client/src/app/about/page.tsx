/* eslint-disable @next/next/no-img-element */
const About = () => {
  return (
    <div className='flex flex-col items-center justify-center px-6 text-white'>
      <div className='py-[150px] text-center'>
        <h1 className='text-white text-[37px] m-0 mb-5 lg:text-[56px] lg:mb-8'>
          About MeshNode
        </h1>
        <div className='flex items-center justify-center text-white lg:w-[80%] lg:m-auto lg:text-[18px] xl:w-[60%]'>
          MeshNode is a decentralized Q&A platform where users ask, answer, and
          engage with the community, promoting knowledge exchange and
          collaboration. Valuable contributions are rewarded, incentivizing
          active participation and fostering insightful discussions. With its
          decentralized infrastructure, MeshNode ensures transparency, security,
          and user control, creating a vibrant ecosystem for decentralized
          community-based knowledge sharing.
        </div>
      </div>
      <div className='pb-[150px]'>
        <h2 className='m-0 mb-10 text-[26px] text-center lg:text-[36px]'>
          What drives MeshNode?
        </h2>
        <div className='grid grid-cols-1 gap-7 sm:grid-cols-2'>
          <div className='rounded-[16px] bg-dark-blue overflow-hidden flex flex-col p-8 items-start justify-start gap-6  sm:w-[300px] lg:w-[400px] xl:w-[550px]'>
            <div className='rounded-[8px] bg-blue w-14 h-14 overflow-hidden shrink-0 flex flex-row items-center justify-center'>
              <img
                className='w-7 h-7'
                alt=''
                src='/60e4a1e73807563947e7fb99-iconsectionabout1cryptotemplatesvg.svg'
              />
            </div>
            <div>
              <h3 className='uppercase m-0 mb-4 text-[18px]'>
                Ask and answer questions
              </h3>
              <p className='flex items-center m-0 text-[16px]'>
                Engage with the community by asking questions and providing
                answers on various topics.
              </p>
            </div>
          </div>
          <div className='rounded-[16px] bg-dark-blue overflow-hidden flex flex-col p-8 items-start justify-start gap-6  sm:w-[300px] lg:w-[400px] xl:w-[550px]'>
            <div className='rounded-[8px] bg-blue w-14 h-14 overflow-hidden shrink-0 flex flex-row items-center justify-center'>
              <img
                className='w-7 h-7'
                alt=''
                src='/60e4a1e7380756fecde7fb9c-iconsectionabout2cryptotemplatesvg.svg'
              />
            </div>
            <div>
              <h3 className='uppercase m-0 mb-4 text-[18px]'>
                see stats and upvotes
              </h3>
              <p className='flex items-center m-0 text-[16px]'>
                Evaluate the quality of content with statistics and upvoting,
                promoting valuable contributions.
              </p>
            </div>
          </div>
          <div className='rounded-[16px] bg-dark-blue overflow-hidden flex flex-col p-8 items-start justify-start gap-6  sm:w-[300px] lg:w-[400px] xl:w-[550px]'>
            <div className='rounded-[8px] bg-blue w-14 h-14 overflow-hidden shrink-0 flex flex-row items-center justify-center'>
              <img
                className='w-7 h-7'
                alt=''
                src='/60e4a1e73807565f69e7fb9b-iconsectionabout3cryptotemplatesvg.svg'
              />
            </div>
            <div>
              <h3 className='uppercase m-0 mb-4 text-[18px]'>get rewards</h3>
              <p className='flex items-center m-0 text-[16px]'>
                Earn incentives and rewards for valuable contributions that
                enhance the community&#39;s knowledge base.
              </p>
            </div>
          </div>
          <div className='rounded-[16px] bg-dark-blue overflow-hidden flex flex-col p-8 items-start justify-start gap-6  sm:w-[300px] lg:w-[400px] xl:w-[550px]'>
            <div className='rounded-[8px] bg-blue w-14 h-14 overflow-hidden shrink-0 flex flex-row items-center justify-center'>
              <img
                className='w-7 h-7'
                alt=''
                src='/60e4a1e73807561d83e7fbc5-iconaboutdrive3cryptotemplatesvg.svg'
              />
            </div>
            <div>
              <h3 className='uppercase m-0 mb-4 text-[18px]'>
                all decentralised
              </h3>
              <p className='flex items-center m-0 text-[16px]'>
                Operates on a decentralized infrastructure, ensuring
                transparency, security, and user control over data.
              </p>
            </div>
          </div>
        </div>
      </div>
      <div className='pb-[150px]'>
        <h2 className='m-0 mb-4 text-[26px] text-center lg:text-[36px] lg:mb-6'>
          Our Team
        </h2>
        <p className='text-[16px] m-0 mb-10 text-center lg:text-lg lg:mb-20 lg:w-[60%] lg:mx-auto'>
          A dedicated group of active learners working together to bring
          innovative solutions and make a positive impact
        </p>
        <div className='flex flex-row flex-wrap gap-[38px] items-center justify-center'>
          <div className='rounded-[16px] bg-dark-blue w-full max-w-[20.44rem] overflow-hidden flex flex-col p-[1.5rem] box-border items-start justify-start gap-[2rem]'>
            <img
              className='rounded-[16px] w-full h-[20rem] object-cover'
              alt=''
              src='/vijay.jpeg'
            />
            <div className='flex flex-col py-[0rem] pr-[3.43rem] pl-[0rem] box-border items-start justify-center'>
              <div className='flex flex-col items-start justify-start gap-[0.75rem]'>
                <b className='tracking-[0.8px] leading-[1.13rem] uppercase'>
                  VIJAY KUMAr
                </b>
                <div className='text-[1rem] tracking-[0.8px] leading-[1.13rem] uppercase'>
                  CORE DEV
                </div>
              </div>
            </div>
          </div>
          <div className='rounded-[16px] bg-dark-blue w-full max-w-[20.44rem] overflow-hidden flex flex-col p-[1.5rem] box-border items-start justify-start gap-[2rem]'>
            <img
              className='rounded-[16px] w-full h-[20rem] object-cover'
              alt=''
              src='/ayush.jpeg'
            />
            <div className='flex flex-col py-[0rem] pr-[3.43rem] pl-[0rem] box-border items-start justify-center'>
              <div className='flex flex-col items-start justify-start gap-[0.75rem]'>
                <b className='tracking-[0.8px] leading-[1.13rem] uppercase'>
                  Ayush i
                </b>
                <div className='text-[1rem] tracking-[0.8px] leading-[1.13rem] uppercase'>
                  CORE DEV
                </div>
              </div>
            </div>
          </div>
          <div className='rounded-[16px] bg-dark-blue w-full max-w-[20.44rem] overflow-hidden flex flex-col p-[1.5rem] box-border items-start justify-start gap-[2rem]'>
            <img
              className='rounded-[16px] w-full h-[20rem] object-cover'
              alt=''
              src='/ayan.jpeg'
            />
            <div className='flex flex-col py-[0rem] pr-[3.43rem] pl-[0rem] box-border items-start justify-center'>
              <div className='flex flex-col items-start justify-start gap-[0.75rem]'>
                <b className='tracking-[0.8px] leading-[1.13rem] uppercase'>
                  Ayan Maiti
                </b>
                <div className='text-[1rem] tracking-[0.8px] leading-[1.13rem] uppercase'>
                  CORE DEV
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
      <div className='pb-[150px]'>
        <h2 className='m-0 mb-4 text-[26px] text-center lg:text-[36px] lg:mb-6'>
          You made it possible for us!
        </h2>
        <p className='text-[16px] m-0 mb-10 text-center lg:text-lg lg:w-[60%] lg:mx-auto'>
          Thanks to their exceptional services, we were able to seamlessly
          integrate reliable data feeds and optimize blockchain development,
          empowering our project&#39;s success.
        </p>
        <div className='flex flex-col sm:flex-row items-center justify-center gap-4'>
          <div className='flex flex-row items-center justify-center rounded-[16px] bg-blue box-border border-2 border-solid border-white gap-[30px] px-8 py-4'>
            <img className='w-14 h-14' alt='' src='/chainlink-white.png' />
            <h3 className='m-0'>Chainlink</h3>
          </div>
          <div className='flex flex-row items-center justify-center rounded-[16px] bg-blue box-border border-2 border-solid border-white gap-[30px] px-8 py-4'>
            <img className='w-14 h-14' alt='' src='/polygon-white.png' />
            <h3 className='m-0'>Polygon</h3>
          </div>
        </div>
      </div>
      <div className='pb-[150px]'>
        <h2 className='m-0 mb-4 text-[26px] text-center  lg:text-[36px] lg:mb-6'>
          Tech Stack
        </h2>
        <p className='text-[16px] m-0 mb-10 text-center  lg:text-lg lg:w-[50%] lg:mx-auto'>
          Leveraging cutting-edge technologies and frameworks to build a robust
          and scalable platform that meets the evolving needs of the users.
        </p>
        <div className='flex flex-row flex-wrap justify-center items-center gap-4 lg:w-[70%] lg:mx-auto xl:w-[80%]'>
          <div className='flex items-center justify-center rounded-[16px] bg-blue w-20 h-20 lg:w-40 lg:h-40'>
            <img
              className='w-16 h-16 lg:w-28 lg:h-28'
              alt=''
              src='/chainlink-white.png'
            />
          </div>
          <div className='flex items-center justify-center rounded-[16px] bg-blue w-20 h-20 lg:w-40 lg:h-40'>
            <img
              className='w-16 h-16 lg:w-28 lg:h-28'
              alt=''
              src='/polygon-white.png'
            />
          </div>
          <div className='flex items-center justify-center rounded-[16px] bg-blue w-20 h-20 lg:w-40 lg:h-40'>
            <img
              className='w-9 h-12 lg:w-14 lg:h-20'
              alt=''
              src='/solidity-white.png'
            />
          </div>
          <div className='flex items-center justify-center rounded-[16px] bg-blue w-20 h-20 lg:w-40 lg:h-40'>
            <img
              className='w-12 h-12 lg:w-20 lg:h-20'
              alt=''
              src='/hardhat-white.png'
            />
          </div>
          <div className='flex items-center justify-center rounded-[16px] bg-blue w-20 h-20 lg:w-40 lg:h-40'>
            <img
              className='w-9 h-10 lg:w-20 lg:h-24'
              alt=''
              src='/chai-white.png'
            />
          </div>
          <div className='flex items-center justify-center rounded-[16px] bg-blue w-20 h-20 lg:w-40 lg:h-40'>
            <img
              className='w-12 h-12 lg:w-20 lg:h-20'
              alt=''
              src='/moccha-white.png'
            />
          </div>
          <div className='flex items-center justify-center rounded-[16px] bg-blue w-20 h-20 lg:w-40 lg:h-40'>
            <img
              className='w-12 h-12 lg:w-20 lg:h-20'
              alt=''
              src='/next-white.png'
            />
          </div>
          <div className='flex items-center justify-center rounded-[16px] bg-blue w-20 h-20 lg:w-40 lg:h-40'>
            <img
              className='w-10 h-10 lg:w-16 lg:h-16'
              alt=''
              src='/ts-white.png'
            />
          </div>
          <div className='flex items-center justify-center rounded-[16px] bg-blue w-20 h-20 lg:w-40 lg:h-40'>
            <img
              className='w-12 h-12 lg:w-20 lg:h-20'
              alt=''
              src='/tailwind-white.png'
            />
          </div>
          <div className='flex items-center justify-center rounded-[16px] bg-blue w-20 h-20 lg:w-40 lg:h-40'>
            <img
              className='w-24 h-16 lg:w-48 lg:h-36 mix-blend-lighten'
              alt=''
              src='/wagmi.png'
            />
          </div>
          <div className='flex items-center justify-center rounded-[16px] bg-blue w-20 h-20 lg:w-40 lg:h-40'>
            <img
              className='w-12 h-12 lg:w-20 lg:h-20'
              alt=''
              src='/alchemy-white.png'
            />
          </div>
          <div className='flex items-center justify-center rounded-[16px] bg-blue w-20 h-20 lg:w-40 lg:h-40'>
            <img
              className='w-12 h-12 lg:w-20 lg:h-20'
              alt=''
              src='/figma-white.png'
            />
          </div>
          <div className='flex items-center justify-center rounded-[16px] bg-blue w-20 h-20 lg:w-40 lg:h-40'>
            <img
              className='w-12 h-12 lg:w-20 lg:h-20'
              alt=''
              src='/notion-white.png'
            />
          </div>
        </div>
      </div>
    </div>
  );
};

export default About;
